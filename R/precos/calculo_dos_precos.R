...
if (calcular_precos_producao) {
  print(paste0("calculando precos de producao do ano ",as.character(ano)))
  A <- m.wio[1:tamanho,1:tamanho]/produto_bruto_matriz
  A[is.infinite(A)] <- 0
  A[is.nan(A)] <- 0
  
  K <- k.composicao/produto_bruto_matriz
  K[is.infinite(K)] <- 0
  K[is.nan(K)] <- 0
  
  D <- m.depreciacao/produto_bruto_matriz
  D[is.infinite(D)] <- 0
  D[is.nan(D)] <- 0
  ...
  b.a0 <- (matrix(lab.usd, ncol = tamanho, nrow = tamanho, byrow = TRUE)*prop_demanda_familias)/produto_bruto_matriz
