...
if (variavel_trabalho == "Tciz_hora") {
  ## método Ochoa internacionalizado - cesta de consumo única para o mundo
  prop_demanda_familias <- as.data.frame(rowSums(m_wio[1:tamanho,col_demanda_final])/sum(m_wio[1:tamanho,col_demanda_final]))
  prop_demanda_familias <- as.matrix(prop_demanda_familias[rep(names(prop_demanda_familias), each = num_setores*num_paises)])
