...
Uma medição científica só recebe `controlled-reused-singleton` quando seu intervalo UTC não se sobrepõe a nenhum outro job, comparador, preparação ou falha da campanha, incluindo tentativas malsucedidas e seeds auxiliares. Chamadas suplementares sem histórico individual recebem um intervalo conservador de exclusão. Executar o desempenho depois de concluir o suplemento evita descarte conservador desnecessário de medições.
