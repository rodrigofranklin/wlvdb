...
| CI | Execuções de `972d9f8`, `ed3fdc0`, `a73d344` e `175e287` concluídas com sucesso. A execução `33939322479`, de `8543089`, ainda estava em andamento na última consulta. |
...
A segunda comparação antecipada WIOD13 terminou com `passed=true` e `comparison_passed=true`, de `02:28:07Z` a `02:31:28Z` (cerca de 201 segundos). SHA-256 de `comparison/comparison.json`: `bf820579c98f74cc7803998dc2030445fdc7dab69e169a1186372118b233fcca`. O wrapper PID `50612` e o R PID `45888` encerraram. Às `02:33:59Z`, foi iniciada a comparação antecipada WIOD16, com o mesmo vínculo, em `early-parity-wiodr16-001/`, wrapper PID `35104`; ainda sem resultado na última consulta. Ambas leem os cálculos já autenticados, não escrevem no estado principal e não repetem ciência. O commit de tooling `8543089` contém a correção; o candidato científico permanece em `972d9f8`.
...
Após os quatro cálculos, a inspeção somente leitura encontrou 8,60 GiB de resultados finais (WIOD13: 0,959 GiB por braço; WIOD16: 3,343 GiB por braço), com C/D livres em aproximadamente 98,28/82,41 GiB. A projeção para 28 resultados é 60,23 GiB. O `Plan` atual superestima o lane serial ao somar uma margem transitória de 2,5 vezes para cada resultado; é necessário separar armazenamento persistente cumulativo do pico transitório de um único job, mantendo o cheque individual e o piso de 10 GiB. Esse ajuste ainda não foi implementado.

A alternativa recomendada para desempenho é usar quatro roots dedicados em C: os quatro cálculos controlados já necessários tornam-se os seeds locais dos 20 recálculos, sem acrescentar quatro cálculos. Cada cadeia deve permanecer no mesmo `project_root`, com fontes autenticadas e vínculo exclusivo de desempenho; não importar os seeds de D alterando sua proveniência. Com fontes (9,28 GiB), resultados projetados (60,23 GiB), margem transitória serial (5,11 GiB) e piso (10 GiB), a reserva projetada é 84,62 GiB. Quatro comparações estritas D↔C deverão comprovar a equivalência dos seeds. Nenhum root C foi criado, arquivo movido/apagado ou medição iniciada nesta inspeção; reavaliar os bytes livres antes de executar.
