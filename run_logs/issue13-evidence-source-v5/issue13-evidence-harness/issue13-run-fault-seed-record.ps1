param(
  [Parameter(Mandatory = $true)][string]$SeedPlanPath,
...
$gitEnvironment = [Environment]::GetEnvironmentVariable(
  'ISSUE13_V5_GIT_EXECUTABLE', 'Process')
if ([string]::IsNullOrWhiteSpace($gitEnvironment) -or
    -not [IO.Path]::IsPathFullyQualified($gitEnvironment)) {
  throw 'ISSUE13_V5_GIT_EXECUTABLE must be an absolute path.'
}
$gitExecutable = [IO.Path]::GetFullPath($gitEnvironment)
if (-not [IO.File]::Exists($gitExecutable)) {
  throw 'ISSUE13_V5_GIT_EXECUTABLE does not identify an existing file.'
}

$seedPlanResolved = Resolve-ExactPath $SeedPlanPath 'Fault seed plan'
$plan = Get-Content -LiteralPath $seedPlanResolved -Raw -Encoding UTF8 |
  ConvertFrom-Json
if ($plan.schema -ne 'wlv-issue13-fault-seed-plan/1' -or
    [bool]$plan.execution_started -or
    [int]$plan.record_count -ne 10 -or @($plan.records).Count -ne 10) {
  throw 'Fault seed plan schema or cardinality is invalid.'
}
if ((File-Hash (Resolve-ExactPath ([string]$plan.source_plan_path) 'Source plan')) -ne
      [string]$plan.source_plan_sha256 -or
    (File-Hash (Resolve-ExactPath ([string]$plan.source_plan_audit_path) `
      'Source plan audit')) -ne [string]$plan.source_plan_audit_sha256 -or
    (File-Hash (Resolve-ExactPath ([string]$plan.import_report_path) `
      'Import report')) -ne [string]$plan.import_report_sha256 -or
    (File-Hash (Resolve-ExactPath ([string]$plan.imported_seed_proof_path) `
      'Imported seed proof')) -ne [string]$plan.imported_seed_proof_sha256) {
  throw 'A fault-seed input changed after plan construction.'
}
$actualCommit = (& $gitExecutable -C ([string]$plan.fault_root) `
  rev-parse HEAD).Trim()
if ($LASTEXITCODE -ne 0 -or $actualCommit -ne [string]$plan.candidate_commit) {
  throw 'Fault worktree commit differs from the seed plan.'
}
$status = @(& $gitExecutable -C ([string]$plan.fault_root) status `
  '--porcelain=v1' '--untracked-files=no')
if ($LASTEXITCODE -ne 0 -or $status.Count -ne 0) {
  throw 'Fault worktree runtime files are not clean.'
}
$records = @($plan.records | Where-Object {
  [string]$_.scenario_id -eq $ScenarioId
})
if ($records.Count -ne 1) {
  throw "Unknown or duplicate fault seed scenario: $ScenarioId"
}
$record = $records[0]
$spec = Resolve-ExactPath ([string]$record.process_spec_path) 'Seed process spec'
if ((File-Hash $spec) -ne [string]$record.process_spec_sha256) {
  throw "Seed process spec changed: $ScenarioId"
}
$seedSpec = Resolve-ExactPath ([string]$record.seed_spec_path) 'Channel seed spec'
if ((File-Hash $seedSpec) -ne [string]$record.seed_spec_sha256) {
  throw "Channel seed spec changed: $ScenarioId"
}
$evidence = [IO.Path]::GetFullPath([string]$record.evidence_directory)
if (Test-Path -LiteralPath $evidence) {
  throw "Seed evidence already exists; runner requires a fresh record: $evidence"
}
New-Item -ItemType Directory -Path $evidence | Out-Null
$monitor = Join-Path $PSScriptRoot 'issue13-monitor.ps1'
& $monitor -SpecPath $spec -EvidenceDir $evidence
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
if (-not (Test-Path -LiteralPath (Join-Path $evidence 'seed-result.json') `
      -PathType Leaf) -or
    -not (Test-Path -LiteralPath (Join-Path $evidence 'process-metrics.json') `
      -PathType Leaf)) {
  throw "Seed did not emit complete evidence: $ScenarioId"
}
Write-Output "Seed passed: $ScenarioId"
exit 0
