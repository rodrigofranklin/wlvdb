...
$plan = Get-Content -LiteralPath $seedPlanResolved -Raw -Encoding UTF8 | ConvertFrom-Json
...
if ((File-Hash (Resolve-ExactPath ([string]$plan.source_plan_path) 'Source plan')) -ne
    ...
    (File-Hash (Resolve-ExactPath ([string]$plan.source_plan_audit_path) 'Source plan audit')) -ne
      [string]$plan.source_plan_audit_sha256 -or
    (File-Hash (Resolve-ExactPath ([string]$plan.import_report_path) 'Import report')) -ne
      [string]$plan.import_report_sha256 -or
    (File-Hash (Resolve-ExactPath ([string]$plan.imported_seed_proof_path) 'Imported seed proof')) -ne
      [string]$plan.imported_seed_proof_sha256) {
...
foreach ($record in @($plan.records)) {
  ...
  if ($seen.ContainsKey($id) -or $seen.ContainsKey($channel)) {
    throw "Duplicate fault seed id/channel: $id / $channel"
  }
  $seen[$id] = $true
  $seen[$channel] = $true
  ...
  if ((File-Hash $spec) -ne [string]$record.process_spec_sha256) {
    throw "Seed process spec changed: $id"
  }
  ...
  if ((File-Hash $seedSpec) -ne [string]$record.seed_spec_sha256) {
    throw "Channel seed spec changed: $id"
  }
  $evidence = [System.IO.Path]::GetFullPath([string]$record.evidence_directory)
  if (Test-Path -LiteralPath $evidence) {
    throw "Seed evidence already exists; reruns are forbidden: $evidence"
  }
  New-Item -ItemType Directory -Path $evidence | Out-Null
  & $monitor -SpecPath $spec -EvidenceDir $evidence
  if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
  if (-not (Test-Path -LiteralPath (Join-Path $evidence 'seed-result.json') -PathType Leaf) -or
      -not (Test-Path -LiteralPath (Join-Path $evidence 'process-metrics.json') -PathType Leaf)) {
    throw "Seed did not emit complete evidence: $id"
  }
  Write-Output "Seed passed: $id"
}
exit 0
